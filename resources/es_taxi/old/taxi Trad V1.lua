...
function StartJob(jobid)
    ...
    jobs.coords.cx[92],jobs.coords.cy[92],jobs.coords.cz[92] = -798.090, 831.699, 204.351
		...
		Wait(2000)
		DrawMissionText("Faites le tour et cherchez des ~h~~y~clients~w~.", 10000)
		onJob = jobid
	end
end
			...
			if DoesEntityExist(jobs.cars[1]) and IsVehicleDriveable(jobs.cars[1], 0) then
				if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
								...
								if jobs.flag[2] == 0 then
									...
									if IsPedSittingInVehicle(jobs.peds[1], jobs.cars[1]) then
										...
										if street[2] ~= 0 and street[2] ~= nil then
											local streetname = string.format("~w~Déposez-moi~y~ %s~w~, prêt de~y~ %s", GetStreetNameFromHashKey(street[1]),GetStreetNameFromHashKey(street[2]))
                  ...
                  DrawMissionText("~g~Vous avez déposé le client !", 5000)
									...
									Wait(8000)
									DrawMissionText("Faites le tour et recherchez un nouveau ~h~~y~clients~w~.", 10000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 90)
								end
							end
					...
					if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), GetEntityCoords(jobs.cars[1]), true) > 30.0001 then
						...
						DrawMissionText("Remontez dans votre voiture pour ~y~continuer~w~. Ou éloignez-vous du taxi pour ~r~arrêter~ la mission.", 1)
					end
				end
			else
				StopJob(1)
